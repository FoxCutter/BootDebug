...
struct DriverInformationBase
{
	uint32_t	Size;
	char *		Name;
	void *		EntryPoint;
	uint32_t	Flags;
	DriverInformationBase * NextDriver;
	DriverInformationBase * PrevDriver;
};

struct SystemBoardDriver
{
	DriverInformationBase Header; 
	char HID[16];					// HID string as defined by ACPI.
};
...
enum DriverInformationFlags : uint32_t
{
	eDriverInformationVenderDevice =		0x00000001, // The Vender/Device information is valid
	eDriverInformationInterfaceDevice =		0x00000002, // This can be used as an interface device if no exact match is found

	eDriverInformationUSBVersion =			0x00010000, // In a USB Driver the Version is valid
	eDriverInformationPCIRevisionID =		0x00010000, // In a PCI Driver the Revsion ID field is valid

	eDriverInformationUSBProtocolSkip =		0x00020000, // In a USB Driver the Protocol field can be ignored if there are no other matches
	eDriverInformationUSBInterfaceIDSkip =	0x00020000, // In a PCI Driver the Interface ID gield can be ignored if there are no other matches

	eBusTypeSystem =						0x00000000, // Connected to the system board directly, no bus
	eBusTypePCI =							0x01000000, // PCI
	eBusTypeUSB =							0x02000000, // USB
};

#pragma data_seg(".DRIVER$XOHCI")
PCIDriver OHCIDriverBlock = { 
	{ 
			sizeof(PCIDriver),					// Size
			"USB1-OHCI",						// Name
			nullptr,							// Function
			eBusTypePCI | eDriverInformationInterfaceDevice	// We are an PCI interface Driver
	}, 
